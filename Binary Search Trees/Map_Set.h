...
public:
	...
	public:
		...
		// must return by ref T& for class Map
...
public:
	...
	// modified insert for Map
	iterator insert(const T& x)
	{
		BinaryNode* p = insert(x, root, root);
		return iterator(p);
	}
...
private:
	...
	BinaryNode* insert(const T& x, BinaryNode*& t, BinaryNode*& par)
	{
		if (t == nullptr)
		{
			...
			return insert(x, t->left, t);  // NEW
		else if (t->element < x)
			return insert(x, t->right, t); // NEW
		else
			return t; // NEW
	}
	...
	BinaryNode* clone(BinaryNode* t) const
	{
		...
		{
			BinaryNode* newnode = new BinaryNode(t->element, clone(t->left), clone(t->right), nullptr);
			return newnode;
		}
	}
